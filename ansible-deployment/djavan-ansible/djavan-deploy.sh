...
#deploy djavan using ansible
echo "Preapring to deploy djavan"
echo ""
if [ -e "/etc/ansible/deploy.yml" ];
then
  echo "deploying djavan application"
  echo ""
  ansible-playbook -v /etc/ansible/deploy.yml
  echo ""
else
  echo "/etc/ansible/deploy.yml does not exist, unable to deploy djavan"
  echo "The ansible playbooks do not exist, please re-run this script. If failure continues, please review /var/log/deploy-djavan.log"
  exit 1
fi